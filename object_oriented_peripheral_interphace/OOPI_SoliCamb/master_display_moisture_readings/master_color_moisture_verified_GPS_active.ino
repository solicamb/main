...
void loop() { 
  ...
  Sensor MySensor(SPI1_NSS_PIN); //Instantiate Sensor object
  ...
  if (MySensor.areYouConnected()) MySensor.issueCommand(RestartMeasurementProcedure);
  ...
  if (down && page == 1) {
    ...
    menuitem++;
    drawMenu(MySensor);
    if (menuitem==3) //only two menu items
    {
      menuitem=1;
      drawMenu(MySensor);
    }      
  ...
  if (middle) {
    ...
    if (page == 1 && menuitem==2) 
    {
      ...
      if (MySensor.areYouConnected())
      {
        ...
        serialnumber = MySensor.getIDNumber();

        display.clearDisplay();
        display.setCursor(0, 0);
        display.print((String)sName);
        display.print("found! Serial number: ");
...
//---------page 1:the main menu page---------------------------------    
  ...
  {    
    ...
    if(MySensor.areYouConnected()){
      display.setTextSize(1);
      display.print("Sensor Not Connected");
...
//---------------page 2: for peripheral sensor---------------------------------- 
  else if (page==2) 
  {
    ...
    display.println((string)sName);

    //If the sensor has requested for a string to be displayed to the user
    if ((MySensor.getCurrentCommandInstruction() == DisplayInstructionAndWait) || (MySensor.getCurrentCommandInstruction() == DisplayInstructionAndWaitForUser)){
      //Get and print instruction to user
        char command[SLAVE_COMMMAND_STRING_LENGTH];
        MySensor.getCurrentCommandString(command);

        display.setCursor(0, 15);
        display.print((String)command);
        display.drawFastHLine(0,10,83,BLACK);
    } else{
      display.setCursor(0, 15);
...
void sensing(Sensor& MySensor){
  ...
  if (!MySensor.issueCommand(BeginMeasurement)){ //If sensor doesn't acknowledge the begin measurement command, restart loop
      ...
      display.setTextColor(BLACK, WHITE);
      display.setCursor(5, 0);
      display.println("Sensor not responding. Check connection.");
      delay(1000);
      return;
    } 
      ...
      //Check for Data
        if (MySensor.isThereData()){
          ...
          //Get single values or arrays based on dimensions
            ...
            MeasurementVectors vectNum = {First,Second,Third};

            for (int i = First; i < (Third+1); i++){

              //Load next vector
                MySensor.getDataVector(vectNum[i],DataVect);
